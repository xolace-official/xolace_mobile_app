...
import { renderListItem, renderSectionHeader, sections } from "@/src/components/search/mock-data";
...
export const SearchModal = () => {
  ...
  const rListContainerStyle = useAnimatedStyle(() => {
    return {
      opacity: interpolate(transitionProgress.value, [1, 1.5], [1, 0], Extrapolation.CLAMP),
      transform: [
        {
          translateY: interpolate(transitionProgress.value, [1, 1.5], [0, 20], Extrapolation.CLAMP),
        },
        {
          scale: interpolate(transitionProgress.value, [1, 1.5], [1, 0.9], Extrapolation.CLAMP),
        },
      ],
    };
  });
